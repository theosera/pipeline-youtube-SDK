...
# A "word" = a maximal run of Latin / Cyrillic / Greek letters. Restricting the
# alphabet is important: Python's generic Unicode-letter class also includes
# kana and CJK, so natural Japanese such as ``API\u3068<Russian>`` would otherwise
# glue the Latin and Russian runs together and corrupt the legitimate Russian.
# The *fold* uses this finer boundary instead of _TOKEN_RE so a
...
def fold_mixed_script_confusables(text: str) -> str:
    ...
    pure-script word. CJK/kana are explicit boundaries and remain outside
    candidate words. Every other word -- pure-Latin, pure-Cyrillic (legitimate
    Russian), and pure-Greek -- is returned unchanged.
...
def fold_markdown_mixed_script_confusables(
    text: str,
    *,
    fold_wikilink_targets: frozenset[str] | set[str] = frozenset(),
) -> str:
    """Fold prose while preserving existing Obsidian link/embed targets.

    A Leader-generated body contains both prose and identifiers copied from
    external filenames, such as ``[[video title#^00-30]]`` and
    ``![[playlist folder/asset.webp]]``. Rewriting those targets would break
    otherwise-valid links because filename-bound external text is intentionally
    detect-only. Text outside links and display aliases is folded normally.

    ``fold_wikilink_targets`` is the narrow opt-in for identifiers generated by
    the same operation. A target is compared without its ``#fragment``; matching
    targets are folded so generated chapter links stay aligned with their folded
    filenames.
    """
    if not text:
        return text

    targets = frozenset(fold_wikilink_targets)
    parts: list[str] = []
    cursor = 0
    ...
    for match in _WIKILINK_RE.finditer(text):
        ...
        parts.append(f"{match.group(1)}{target}{separator}{alias}{match.group(3)}")
        cursor = match.end()

    parts.append(fold_mixed_script_confusables(text[cursor:]))
    return "".join(parts)
