...
- **回答末尾に JST 完了時刻を出す**: 各回答の最後に必ず
  `date -u -d '+9 hours' '+%Y-%m-%d %H:%M:%S JST' 2>/dev/null || TZ=Asia/Tokyo date '+%Y-%m-%d %H:%M:%S JST'`
  を実行し、その出力 (JST の完了時刻) を 1 行で表示する。JST は UTC+9 固定 (DST 無し) なので、
  Asia/Tokyo の zoneinfo を持たない最小コンテナでも UTC+9 算術で正しく求まる (素の
  `TZ=Asia/Tokyo date` は zoneinfo 欠如時に UTC を JST と誤表示する)。時刻はコマンド出力なので
  日本語化の対象外。各リポの `.claude/hooks/jst-timestamp.sh` (UserPromptSubmit フック) が毎ターン
  本ルールを再注入するが、フックが無い環境でも本ルールを厳守する。
