...
- **Stage 01 の文字起こしは現状速度が遅い**（本パイプラインの 01–04 のボトルネック）。
  **本 PR マージ後に別 PR**で 01 を刷新予定（設計合意済み）:
  1. まず**速度優先（精度度外視）で高速に文字起こし**（Obsidian の YTranscriptor 流。
     ただし時間軸の刻み方が現リポと異なる点は要正規化）。
  2. その後 **web search で誤変換の用語を訂正**。誤変換検知には **Opus または Sonnet（拡張
     思考）＋ web search** を使う。
  3. **01 はトランスクリプトまで**（訂正済み・タイムスタンプ付き）に留める。まとめ・知見抽出は
     **02** に時間軸対応を加えて実装する（ステージ境界を維持し 03/04/05 と整合）。
