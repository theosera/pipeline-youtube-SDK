...
Options:
  ...
  --handson                       Build a step-by-step hands-on tutorial from
                                  ONE long-form talk video URL (slide-based
                                  presentation). Writes per-step notes + MOC
                                  under 'Permanent Note/09_YouTube学習
                                  _Session_only'. Reads config.handson.json by
                                  default. See docs/handson-mode.md.
...
- **`--model`** (default `sonnet`): Stage 02 (要約) / 04 (学習材料) / 05 (synthesis) が使う LLM モデル。`sonnet` / `haiku` / `opus` のエイリアス、または full ID (`claude-sonnet-4-6` 等)。
