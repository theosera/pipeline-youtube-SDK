...
- **隠蔽 (homoglyph / 不可視・bidi) 防御** — `services/confusables` (上流と同一実装)。(1) fetch 境界 (`playlist` の `fetch_metadata` → `analyze_filename_text`) が**唯一の alert 記録点**で、取得タイトルの不可視/双方向/ゼロ幅文字および Latin×Cyrillic/Greek の混在スクリプト (古典的 homoglyph 偽装) を**検出**して alert に記録する (**検出のみ** — emit する `VideoMeta` は生タイトルのままで、ここでは除去しない)。不可視文字の**実際の除去**は下流のファイル名・frontmatter チョークポイント (`sanitize_title_for_filename` / `build_frontmatter` → `strip_invisibles`) が担い、そこでは混在スクリプト検出も再 alert もしない。(2) LLM 出力 (`providers/registry.py` 経由の OpenAI/Claude が稀に吐く Cyrillic/Greek lookalike) を vault へ書き出す**直前**に、混在スクリプトの letter-run 語について、マップ済みの Cyrillic/Greek lookalike を**すべて** Latin へ **fold** する (`fold_mixed_script_confusables`) — Stage 02 summary と Stage 05 synthesis (chapter / MOC) の LLM 出力経路**のみ** (Stage 01 の字幕本文は fold 対象外)、`validate_chapter_body` の HTML 無害化の**前段**に配置。fold は決定論・冪等・混在 letter-run 語限定で、「無変更」の保証は**純スクリプト run** (日本語・純キリル/ギリシャ語、および URL/markdown 内の正当な非ラテンは letter-run 分割で純スクリプト語として生存) に対して成立する。詳細は上流 README のセキュリティ層を参照。
