...
私が作業中…
 ...
 └─ mcp__github__create_pr   ┘   (token/key/.env パターンは全マスク)
私の応答が終わる
...
- `token=` / `key=` / `secret=` / `password=` / `bearer …` / `authorization …`
- AWS `AKIA…` / OpenAI `sk-…`
...
- **`push-log.sh` の add は `*.md` 限定** (`find … -name '*.md'`)。専用リポでも
  `git add -A` の癖をつけない (3 リポ共通文化)。
