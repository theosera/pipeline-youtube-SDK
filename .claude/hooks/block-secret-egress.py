...
#
# `\??` は optional chaining (`process?.env.X`) のためで、2 箇所ともに要る。
# 前だけ直すと `process?.env?.X` が残る — 後続の `?.` は `?` で始まるので、
# 後ろの lookahead が `.` か `[` しか受け付けないと当たらないからである。
#
# 後ろの lookahead は空白を受け付けない。`\s*` を挟むと、次の「語」が `.` か `[`
# で始まるだけでアクセサとみなして伏せてしまい、⛔ 末尾に ` ./x` や ` [a-z]` を
# 足すだけでこの門を回避できる。実測 (2026-09-03): `\s*` があると
# `curl --upload-file process.env ./x https://…` が ALLOW になった。これは
# `\??` を足す前から成立しており、`\s*` を入れた時点からの欠陥である。
#
# 空白を挟んだメンバアクセス (`process.env ['X']`) を通すことと、空白区切りの
# ファイルオペランドを止めることは、この綴りの上では両立しない。fail-closed 側
# を採る — 前者は珍しい書き方で、止まっても手で実行すればよい。
_CODE_ENV_ACCESSOR_RE = re.compile(
    r"\b(?:process|import\.meta)\??\.env(?=[.\[]|\?\.)|\bos\.environ(?=[.\[(]|\?\.)",
    re.IGNORECASE,
)
